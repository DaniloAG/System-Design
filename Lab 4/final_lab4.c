...
void get_pathname(char* uri, char* hostname, char* pathname);
void get_hostname(char* uri, char* hostname);
int is_get_request(char* request);
void error_get(int connfd, char* request);
void forward_request(char* request, int request_len, char* buf, rio_t rio, struct sockaddr_in* clientaddr, int connfd);
...
void process_request(int connfd, struct sockaddr_in* clientaddr) {	
	...
	if (!is_get_request(request)){
		error_get(connfd, request);
		return;
	}
	else{
		forward_request(request, request_len, buf, rio, clientaddr, connfd);
	}
}

int is_get_request(char* request) {
    return (strstr(request, "GET") != 0);
}
...
void forward_request(char* request, int request_len, char* buf, rio_t rio, struct sockaddr_in* clientaddr, int connfd){
	...
	get_hostname(request_uri, hostname);
	get_port(request_uri, hostname, &port);
	get_pathname(request_uri, hostname, pathname);
